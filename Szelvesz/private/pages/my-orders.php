...
<?php if(!empty($myPassiveOrders)): ?>
    <table class="table table-dark table-hover table-striped table-borderless align-middle mx-auto mt-4 text-center ">
            <thead>
            <tr>
                <th scope="col" colspan="9" class="text-primary">Rendelési előzményeim</th>
            </tr>
            <tr>
                <th></th>
                <th scope="col">Kép</th>
        ...
        <?php $counter = 1; ?>
        <?php foreach($myPassiveOrders as $myOrder): ?>
            <?php $car = db_fetch('cars', 'id LIKE :car_id', [':car_id' => $myOrder['car_id']]);?>
            <tr>
                <th scope="row"><?= $counter?>.</th>
                <td> <img src="<?= $car['image'] ?>" class="img-fluid rounded mx-auto d-block"  alt=""></td>
...
<table class="table table-dark table-hover table-striped table-borderless align-middle mx-auto mt-5 text-center ">
    <thead>
    <tr>
        <th scope="col">Önnek nincs már teljesített rendelése!</th>
    </tr>
    </thead>
</table>
